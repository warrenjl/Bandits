...
for(i in 1:n_sims){
   ...
   for(j in 1:n_days){
      ...
      if(counter <= length(active_set)){
        ...
        y_se[i,j]<-y[g_se[j]]
        r_se[i,j]<-max(lambda_true*p_true) -
                   (lambda_true*p_true)[g_se[j]]
        counter<-counter +
                 1
      ...
      if(j <= n_locs){
        ...
        g_eg[j]<-j
        y_eg[i,j]<-y[g_eg[j]]
        r_eg[i,j]<-max(lambda_true*p_true) -
                   (lambda_true*p_true)[g_eg[j]]
      ...
      if(j <= n_locs){
        ...
        g_ucb[j]<-j
        y_ucb[i,j]<-y[g_ucb[j]]
        r_ucb[i,j]<-max(lambda_true*p_true) -
                    (lambda_true*p_true)[g_ucb[j]]
      ...
      g_rs<-sample(c(1:n_locs),
                   size = 1)
      y_rs[i,j]<-y[g_rs]
      r_rs[i,j]<-max(lambda_true*p_true) -
                 (lambda_true*p_true)[g_rs]
      ...
      for(k in 1:length(ee_times)){
         ...
         if(j <= ee_times[k]){
           ...
           y_ee[[k]][i,j]<-y[g_ee[k]]
           r_ee[[k]][i,j]<-max(lambda_true*p_true) -
                           (lambda_true*p_true)[g_ee[k]]
      ...
      y_ts[i,j]<-y[g_ts]
      r_ts[i,j]<-max(lambda_true*p_true) -
                 (lambda_true*p_true)[g_ts]
      ...
      r_rts_p[i,j]<-max(lambda_true*p_true) -
                    (lambda_true*p_true)[g_rts_p]
      ...
      r_rts_nb[i,j]<-max(lambda_true*p_true) -
                     (lambda_true*p_true)[g_rts_nb]
      ...
      ###################################
      y_c[i,j]<-y[g_c]
      r_c[i,j]<-max(lambda_true*p_true) -
                (lambda_true*p_true)[g_c]
